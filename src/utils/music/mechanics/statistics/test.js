...
describe('getPitchCountsInPool', () => {
    ...
    it('returns pitches for partial pool', () => {
        expect(getPitchCountsInPool(
            new Set(MOCK_POOL_LIST)),
        ).toStrictEqual([2, 3, 1, 4, 1, 2, 6, 1, 1, 2, 1, 2])
    })

    it('returns pitches for initial standard pool', () => {
        expect(getPitchCountsInPool(
            getInitialStandardPool()),
        ).toStrictEqual([8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8])
    })

    it('returns pitches for initial extended pool', () => {
        expect(getPitchCountsInPool(
            getInitialExtendedPool()),
        ).toStrictEqual([11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11])
    })
})
